...
* **Visual Snapshots (Playwright)**: Уникальный механизм захвата финальных моментов игры через headless-браузер прямо в Docker-контейнере. Автоматически дифференцирует обычный финиш (`deskTopSnapShot`) и фиксацию нового рекорда игрока (`deskTopSnapShotBest`). Снимки асинхронно сохраняются в MongoDB.
* **Save & Resume (Прерывание и восстановление)**: Возможность полной сериализации состояния игры в MongoDB. При вызове `/restart` система запрашивает у Mongo сохраненный объект `SavedGame`, восстанавливает In-Memory стейт сессии и бесшовно возобновляет рендеринг.
* **Высокопроизводительный игровой цикл**: Разделение ввода-вывода. Команды управления фигурой (поворот, сдвиги) обрабатываются синхронно в оперативной памяти для нулевого пинга. Падение фигуры завязано на реактивный планировщик `TaskScheduler.scheduleAtFixedRate`.
* **Полная асинхронность и неблокирующий IO**: Все внешние обращения к микросервисам баз данных обернуты в `CompletableFuture.thenAccept` и делегированы виртуальным потокам, освобождая основные потоки обработки STOMP-сообщений.
...
Вместо жесткой привязки IP-адресов микросервисов, приложение использует динамический поиск эндпоинтов gRPC через **Eureka Service Discovery** (`discovery:///`).

### Ключевые сетевые решения:
* **Custom Port Resolution (`discovery-stub=true`)**: Позволяет gRPC-клиенту автоматически извлекать кастомный gRPC-порт удаленного микросервиса из метаданных Eureka (по ключу `grpc.port`), разделяя HTTP и gRPC трафик.
* **Plaintext Negotiation**: Внутри контура микросервисов отключено шифрование SSL (`negotiation-type=plaintext`), что полностью решает проблему совместимости протоколов `HTTP/2 Preface` и увеличивает скорость передачи данных.
* **Таймауты и Deadlines**: Для стабильности экосистемы установлены строгие ограничения времени ожидания ответов. На транзакционные операции (`game-service`, `users-service`) выделено **5 секунд**, а на передачу тяжелых скриншотов (`mongo-service`) — увеличенный лимит в **10 секунд**.
* **Размер Пакета (Max Inbound Size)**: Максимальный размер входящего сообщения для Mongo-клиента увеличен до **10 МБ** (`10485760` байт) для беспрепятственной прокачки бинарных снимков экрана высокого разрешения.
...
* **Асинхронная агрегация данных**: Метод `getGameData` выполняет вызов удаленной процедуры `GetGameScore` внутри виртуального потока через `CompletableFuture.supplyAsync`. Результат конвертируется в плоский JSON-объект, адаптированный под логику фронтенда. При сбоях связи перехватчик исключений возвращает безопасную заглушку `{}`, исключая падение UI.

* **Потоковый маппинг**: Метод `getAllGames` транслирует массив структур `repeated GameResponse` из gRPC-канала напрямую в нативный коллекционный тип `java.util.List`. На этапе стрим-обработки происходит бесшовное приведение типов между `int64` (Protobuf) и `Long` (Java).

* **Гибридная архитектура (gRPC + REST)**: Система разделяет протоколы по типу нагрузки. Чтение данных и фиксация рекордов (`doRecord`) переведены на высокоскоростной gRPC. При этом тяжелые каскадные административные удаления (`deleteGameData`) выполняются через синхронный REST-клиент **OpenFeign**, снижая накладные расходы.

* **Изоляция игрового цикла**: Метод сохранения рекордов помечен аннотацией `@Async`, благодаря чему отправка результатов в базу данных выполняется в фоновом режиме и никак не влияет на плавность графики и тайминги ввода активных игроков.
...
* **Оптимизированная передача бинарных данных**: Загрузка скриншотов игрового процесса (`loadSnapShotIntoMongodb`) и аватаров (`loadMugShotIntoMongodb`), а также их скачивание (`downloadBytes`) происходят в чистом бинарном виде. Массивы байтов оборачиваются в `ByteString.copyFrom(data)`, минуя ресурсоемкое Base64-кодирование, что экономит процессорное время на стыке сервисов.

* **Сериализация игровых матриц**: Система выполняет взаимную конвертацию типов данных при сохранении игрового состояния. Двумерный массив игрового поля `char[][]` трансформируется в плоский список строк `List<String>` для gRPC-пакета, а при вызове `gameRestart` восстанавливается обратно в исходную матрицу через `String::toCharArray`.

* **Гибридный сетевой контур**: По аналогии со статистикой, сервис эффективно сочетает транспортные протоколы. Манипуляции с файлами и слепками экранов переведены на gRPC. Очистка игровых дампов и подготовка коллекций под нового пользователя вынесены в декларативный REST-клиент **OpenFeign** (`MongoFeignClient`).

* **Гибридная асинхронность и изоляция потоков (Loom + REST)**: Доставка контента пользователю разделена на два независимых стрима. Текстовый профиль шлется через легковесные WebSocket-сообщения, где фоновые gRPC-вызовы изолированы в ручном пуле виртуальных потоков (loomExecutor). Тяжелая же графика (скриншоты и аватары) запрашивается фронтендом асинхронно через параллельные HTTP-запросы (fetch). На стороне бэкенда обработка этих HTTP-запросов автоматически подхватывается встроенными виртуальными потоками веб-сервера Tomcat, что полностью предотвращает блокировку физических потоков ОС во время сетевого ожидания gRPC-ответов от базы данных.

* **Предохранители I/O операций**: Чтение локальных файлов скриншотов перед отправкой изолировано блоками перехвата исключений `IOException`. В случае сбоя файловой системы или gRPC-канала, ошибки логируются через `Slf4j`, не прерывая выполнение основного потока игры.

---

## 🔐 Интеграционный слой Авторизации и Управления Пользователями

Связь с центром авторизации и выдачи токенов (`USERS-microservice`) инкапсулирована в сервисе `UsersServiceImpl` и использует gRPC-клиент на базе заглушки `UserServiceGrpc.UserServiceBlockingStub`.
...
* **Глубокая конвертация графа моделей**: Встроенные мапперы (`mapToAppModel` / `mapToMsg`) осуществляют двустороннее преобразование доменных моделей Java в бинарные структуры Protobuf (`UserMsg`, `RoleMsg`). Сложные вложенные коллекции, такие как `Set<Roles>`, автоматически трансформируются в оптимизированные gRPC-массивы `repeated` и обратно с сохранением идентификаторов и наименований прав доступа.
...
## 📺 Слой Визуализации и Трансляции Данных на Фронтенд
...
* **Изоляция сетевого трафика через DTO**: Сервис полностью исключает передачу тяжелых доменных моделей (`State`) по сети. Перед отправкой данные агрегируются в ультралегкие неизменяемые Java-рекорды (`GameStateDTO`, `PlayerHelloDTO`). Двумерный массив стакана `char[][]` динамически рассчитывается методом `drawTetraminoOnCells` непосредственно в момент отправки, снижая нагрузку на пропускной канал.

* **Адресная WebSocket-доставка (Point-to-Point)**: Использование метода `convertAndSendToUser` гарантирует строгую конфиденциальность игровых сессий. Парсинг составного токена `destinationId` на `userId` и `username` позволяет отправлять матрицы ходов, экраны сохранения сессий (`/queue/stateSaved`) и приветственные пакеты (`/queue/gameObjects`) строго в персональный сессионный канал конкретного игрока.

* **Разделение реактивных каналов**: Бизнес-логика игры распределяет типы событий по специализированным WebSocket-очередям. Обычный игровой цикл транслируется в `/queue/stateObjects`, в то время как финальный кадр игры (Game Over) уходит в изолированный канал `/queue/stateFinal`. Это позволяет фронтенду мгновенно переключать экраны и интерфейсы без дополнительного парсинга флагов внутри JSON.

* **Отказоустойчивость UI-потока**: Все методы снабжены защитными проверками на null-состояния (`state == null`). В случае попытки отрисовать несуществующую или уже удаленную администратором сессию, система логирует событие в стандартный вывод и безопасно прерывает цепочку выполнения, не вызывая каскадных падений WebSocket-брокера и не разъединяя других активных пользователей.
