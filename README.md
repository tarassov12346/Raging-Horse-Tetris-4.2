# 🧩 Raging Horse Tetris 4.2: High-Load Distributed Engine

Высокопроизводительная игровая экосистема, спроектированная на базе микросервисной архитектуры и оптимизированная под сверхнизкие сетевые задержки (*Ultra-low latency*) в условиях пиковых конкурентных нагрузок.

Текущий репозиторий содержит **центральный игровой движок (`Raging-Horse-Tetris-4.2`)** — Stateless-ядро системы, координирующее реактивное взаимодействие пользователя по WebSocket, In-Memory вычисления в распределённом кластере и отказоустойчивую gRPC-трансляцию медиа-артефактов в инфраструктуру баз данных.

Архитектура модуля полностью адаптирована под стандарты **Java 21 (Project Loom)**, переведена на императивно-неблокирующий стиль обработки ввода-вывода и изолирована от нативного *Thread Pinning*, что обеспечивает линейное горизонтальное масштабирование при минимальном потреблении ресурсов памяти CPU и Heap.
...
Система реализует сквозную бесшовную авторизацию сессий в неблокирующем стиле, полностью доверяя внешнему периметру безопасности шлюза.

* **Этап Handshake (UserHandshakeInterceptor)**: Когда клиент инициирует соединение по адресу `/websocket`, интерцептор перехватывает HTTP-заголовки `X-User-Id` и `X-User-Name`, которые автоматически пробросил Spring Cloud Gateway, и транслирует их во внутренние атрибуты WebSocket-сессии.
* **Этап STOMP CONNECT (ChannelInterceptor)**: На уровне канального интерцептора входящего потока `preSend` при обработке команды `CONNECT` выполняется безопасное извлечение метаданных пользователя. На их основе генерируется кастомный составной `Principal` вида `userId:username`, инкапсулированный в `UsernamePasswordAuthenticationToken` и привязываемый к сессии.
* **Loom-Safety и Защита от NPE**: Из сетевого контура полностью удален синхронный вывод `System.out` и переведен на асинхронный логгер `SLF4J`, что гарантирует нулевой Thread Pinning на Carrier-потоках сокетов. Внедрены защитные барьеры на валидацию мапы `getSessionAttributes()`, полностью исключающие генерацию `NullPointerException` при несанкционированных подключениях в обход шлюза.
* **Маршрутизация сообщений (SimpleBroker)**: Настроен резидентный брокер сообщений со строгим разделением персональных очередей сессий игроков (`/queue`), общих широковещательных топиков результатов (`/topic`) и префиксов пользовательских назначений (`/user`, `/app`).


### 🔒 Сетевая Безопасность Движка (SecurityConfig)

Поскольку единая точка входа `Gateway-microservice` полностью берет на себя валидацию JWT-токенов, аутентификацию пользователей и очистку внешнего трафика, сам игровой модуль освобожден от избыточных проверок на базе Spring Security 6.x:

* **Императивное отключение CSRF**: Защита от межсайтовой подделки запросов отключена декларативно через современный DSL-лямбда синтаксис `csrf(csrf -> csrf.disable())`, что снижает накладные расходы на валидацию токенов внутри игровых WebSocket-сессий.
* **Сквозной доверенный доступ**: Для всех входящих HTTP и WebSocket запросов активировано неблокирующее правило `authorizeHttpRequests(auth -> auth.anyRequest().permitAll())`. Система полностью делегирует верификацию прав шлюзу, исключая повторные блокирующие I/O вызовы к базам данных внутри контура движка.
* **Контурная изоляция**: Безопасность данных и игровых сессий гарантируется физической и сетевой изоляцией микросервиса внутри приватной виртуальной сети Docker/Kubernetes, недоступной для прямого внешнего сопряжения в обход API Gateway.
...
## 🔄 Балансировка и Сериализация (Network Transport Контур)

Для организации неблокирующего межсервисного обмена данными и сериализации легковесных DTO-пакетов задействован оптимизированный контур сетевого транспорта.

* **Loom-Safe Балансировка Нагрузки (RestTemplate)**: Бин `RestTemplate` снабжен аннотацией `@LoadBalanced` для декларативного выполнения HTTP-запросов к соседним микросервисам через имена в Eureka Service Registry. Клиент принудительно переведен на фабрику `JdkClientHttpRequestFactory` на базе нативного `java.net.http.HttpClient` [312:user10]. Это исключает появление Thread Pinning при синхронных межсервисных вызовах, позволяя виртуальным потокам эффективно парковаться на уровне JVM во время ожидания сетевых ответов.
* **Каноническая Сериализация Кадров (Jackson)**: Настройка Спрингового `ObjectMapper` переведена на декларативный кастомайзер `Jackson2ObjectMapperBuilderCustomizer`. Это обеспечивает правильный порядок инициализации бинов в Spring Boot 3.4, гарантирует нативную поддержку неизменяемых Java 21 рекордов (`GameStateDTO`) и бесшовную интеграцию модуля `JavaTimeModule` для сериализации временных меток `LocalDateTime` без накладных расходов памяти.

## 🛠 Технологический Стек Модуля `Raging-Horse-Tetris-4.2`

Архитектура игрового движка построена на базе современного реактивно-императивного стека Java-технологий, оптимизированного под сверхнизкие сетевые задержки (*Ultra-low latency*) и высокие конкурентные нагрузки:

* **Core Platform & Concurrency**: Java 21 (виртуальные потоки **Project Loom** с поддержкой неблокирующего паркинга I/O), **Spring Boot 3.4.2** (Jakarta EE 10, управление жизненным циклом бинов через функциональные кастомайзеры), **Spring Cloud 2024.0.0** (Eureka Discovery Client, отказоустойчивый распределенный транспорт OpenFeign, встроенный декларативный `Spring Cloud LoadBalancer`).
* **Network & API Interconnect**: **Spring Boot Message Broker** (полнотекстовый брокер сообщений STOMP по WebSocket с поддержкой Point-to-Point маршрутизации сессий и изолированных каналов результатов), высокопроизводительный транспорт **gRPC** (на базе `grpc-client-spring-boot-starter 3.1.0.RELEASE` и `grpc-netty-shaded` для неблокирующего межсервисного обмена метаданными и рекордами), REST API (на базе оптимизированной фабрики `JdkClientHttpRequestFactory` для исключения Thread Pinning).
* **In-Memory & Distributed Cache**: **Hazelcast Platform 5.5** (встроенная интеграция со Spring). Распределенная мапа сессий настроена в реактивном формате `InMemoryFormat.OBJECT` и усилена локальным ближним кэшем **Near Cache** на 5000 элементов с политикой вытеснения **LRU**, что снижает задержки чтения на порядок.
* **Automation & Headless Rendering**: **Playwright Java 1.30.0** (инкапсулированный движок **Chromium** для фонового захвата финальных игровых моментов). Весь ввод-вывод автоматизации аппаратно изолирован в выделенном пуле платформенных потоков `playwright-render` для защиты Carrier-потоков веб-сервера.
* **Data Processing & Build**: **Protobuf-Java** (сериализация бинарных gRPC-сообщений), **Jackson JSON** (кастомизированный через `Jackson2ObjectMapperBuilderCustomizer` с нативной поддержкой Java 21 рекордов и модуля `JavaTimeModule`), **Maven** (с расширением `os-maven-plugin` для детерминированной кроссплатформенной компиляции `.proto` файлов).
...
Управляющие команды сокетов обрабатываются изолированно в кастомном пуле `[loom-*]`. Координация игровых сессий выполняется по принципу Point-to-Point маршрутизации:

* `/hello` — Инициализация сессии, генерация стартового In-Memory стейта в Hazelcast, подготовка структур в MongoDB и трансляция глобальной gRPC-статистики.
* `/{moveId}` — Игровой контроллер управления геометрией: `start` (запуск `TaskScheduler` планировщика автопадения), `1` (поворот), `2` (влево), `3` (вправо), `4` (мгновенный Hard Drop). Размеры поля зафиксированы в свойствах: **12x20**.
* `/save` — Сериализация текущей матрицы стакана, остановка таймера падения и экспорт дампа сессии в MongoDB.
* `/restart` — Извлечение сохраненной сессии из MongoDB по имени игрока и восстановление игрового процесса.
* `/snapShot` — Инициирует асинхронный рендеринг игрового поля через Playwright, последовательно обновляет скриншоты в MongoDB и рассчитывает статус рекорда с использованием неблокирующих цепочек CompletableFuture, изолированных в распределенном виртуальном пуле loomExecutor.
* `/record` — Фиксация и отправка финального счета в PostgreSQL через gRPC-клиент.
* `/profile` — Запрос статистики попыток и лучшего счета пользователя из распределенной базы данных.
* `/upload` — Загрузка кастомного аватара пользователя (`mugShot`) в формате Base64 строки в репозиторий.
* `/admin` — Пакетный бродкаст списков пользователей и результатов всех матчей в общие топики WebSocket-брокера.
* `/admin/{targetUserId}` — Каскадное удаление аккаунта: очистка медиа-файлов в Mongo (`mugShot`, скриншоты), удаление In-Memory сессий и деактивация профиля в сервисе `USERS-microservice`.

### 🌐 HTTP REST Эндпоинты (`@GetMapping`)

Все REST-эндпоинты требуют обязательной передачи идентификатора пользователя в HTTP-заголовке `X-User-Id` для верификации шлюзом. Обработка входящего HTTP-трафика автоматически распределяется между виртуальными потоками Tomcat `[virtual-*]`, обеспечивая неблокирующий стриминг медиа-данных на фронтенд напрямую из MongoDB:

* `/getPhoto` — Потоковый HTTP-стриминг текущего аватара игрока (`image/jpeg`) в браузер пользователя.
* `/getSnapShot` — Стриминг актуального скриншота финального экрана последней завершенной игры.
* `/getSnapShotBest` — Стриминг скриншота экрана лучшего персонального результата.

*Примечание: Межсервисный обмен тяжелыми бинарными массивами (изображениями и скриншотами) полностью изолирован внутри высокопроизводительных gRPC-каналов Netty. Это исключает оверхед HTTP-протокола, а легковесная сериализация Protobuf в связке с виртуальными потоками гарантирует максимальную пропускную способность транспортного контура без фиксации Carrier-потоков.*


## 🚀 Сборка, Генерация Кода и Локальный Запуск

Сборочный конвейер проекта полностью адаптирован под стандарты Java 21 и использует возможности автоматического мета-моделирования gRPC интерфейсов.

### 📋 Системные требования:
* **Среда исполнения**: Установленная **JDK 21** (с поддержкой Project Loom).
* **Инструмент сборки**: Установленный **Maven 3.9+**.
* **Инфраструктурное окружение**: Запущенный и доступный сервер регистрации микросервисов `EUREKA-SERVER` (порт по умолчанию: `1111`).

### 🛠️ Компиляция проекта и кроссплатформенная генерация gRPC

Благодаря интеграции расширения `os-maven-plugin` и плагина `protobuf-maven-plugin`, сборка проекта автоматически адаптируется под операционную систему разработчика (Windows, Linux, macOS x86/Arm) [312:user10]. Расширение динамически вычисляет маркер архитектуры (`os.detected.classifier`) и скачивает нативный компилятор Protobuf.

Для генерации сетевых gRPC-классов, мапперов моделей и полной компиляции исходного кода выполните базовую команду в корне проекта:
```bash
mvn clean compile
```
*Примечание для IDE: Сгенерированные Java-классы gRPC-клиентов автоматически складываются в директорию `target/generated-sources/protobuf/`. Современные среды разработки (IntelliJ IDEA) автоматически подхватывают этот путь как Root-директорию исходных кодов при импорте Maven-модели.*

### 🏃‍♂️ Локальный запуск движка (Spring Boot)

Для запуска игрового движка в режиме локальной отладки выполните команду:
```bash
mvn spring-boot:run
```
*Инженерная особенность старта: В плагине `spring-boot-maven-plugin` программно зашиты необходимые аргументы модульности JVM (`--add-opens java.base/sun.nio.ch=ALL-UNNAMED` и др.). Это гарантирует неблокирующий низкоуровневый доступ In-Memory Grid кластера Hazelcast к оперативной памяти в среде выполнения Java 21 без необходимости ручной проброски флагов виртуальной машины разработчиком.*

## 🧪 Запуск Core Юнит-Тестов (JUnit 5)

В проекте реализован изолированный блок юнит-тестов для верификации физики игрового движка тетриса (схлопывание заполненных рядов, расчет очков, механика падения до дна стакана, сдвиги к стенам и повороты матриц фигур).

Тесты переведены на современный стек **Spring Boot 3.4.x** и **Java 21**.

### 💡 Инженерная особенность сборки:
Тесты спроектированы в **полностью изолированном режиме** (Самодостаточны). На время прогона тестов фоновый сетевой клиент Discovery-сервиса автоматически отключается (`eureka.client.enabled=false`), а распределенный кэш Hazelcast и планировщики Loom-тасок изолируются через Mockito-бины.
* **Для успешного выполнения тестов НЕ ТРЕБУЕТСЯ запускать Eureka Server, Mongo или другие микросервисы окружения.**

### Требования для запуска
* Установленная в системе **JDK 21** (переменная среды `JAVA_HOME` должна указывать на вашу JDK 21).

### Команды для запуска

Используйте встроенный Maven Wrapper проекта для гарантированной сборки без необходимости предустановки локального Maven. Откройте терминал в корневой папке проекта и выполните соответствующую команду для вашей ОС:
...
### Сохранение полного лога сборки в файл
Если вам необходимо запустить тесты с одновременным выводом логов в консоль и сохранением **абсолютно полного отчета** (включая финальную статистику компиляции и плашку `BUILD SUCCESS`) в текстовый файл, используйте команду:

* **Windows (PowerShell):**
  ```powershell
  .\mvnw test | Tee-Object -FilePath "full-build-report.log"
  ```
* **Linux / macOS:**
  ```bash
  ./mvnw test | tee full-build-report.log
  ```
Файл отчета будет автоматически создан в корневой директории проекта.
