...
public class DisplayServiceImpl implements DisplayService {
    ...
    public void sendStateToBeDisplayed(PlayGameService playGameService, GameService gameService, ScheduledExecutorService service, SimpMessagingTemplate template, String userId) {
        playGameService.setState(playGameService.createStateAfterMoveDown(playGameService.getState(userId), service, gameService,userId), userId);
        char[][] cellsToBeDisplayed = playGameService.drawTetraminoOnCells(playGameService.getState(userId));
        State stateToBeSent = playGameService.buildState(playGameService.buildStage(cellsToBeDisplayed, playGameService.getState(userId)), playGameService.getState(userId).isRunning(), playGameService.getState(userId).getGame());
